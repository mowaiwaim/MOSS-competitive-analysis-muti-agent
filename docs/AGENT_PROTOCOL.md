...
第五版核心协作通过 SQLite 表和 JSON 字段表达，不依赖 Agent 间自由文本聊天。新增问卷设计、问卷链接和访谈提纲仍归属“访谈/问卷整理 Agent”，并保留来源、Trace 和降级原因。
...
- 访谈提纲由豆包生成；未配置或调用失败时降级为本地提纲模板，并把降级原因写入 Agent 日志。
...
- 豆包或 mock Provider 输出必须通过 Schema 校验。
...
- 配置豆包时，可调用豆包进行二次复核；模型结果必须作为结构化质检意见记录，不得替代规则校验。
...
- 报告中区分事实、推断和不确定性；“建议与结论”模块本轮暂不展示，配置豆包时只改写输入模块，不新增未输入模块。
